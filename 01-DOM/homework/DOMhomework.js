...
// Tu código acá:
document.querySelector('#createdBy').innerHTML = 'Aplicación creada por Eliz Longart'

/* Otra forma:
Let creadoPor = document.querySelector('#createdBy')
creadoPor.innerHTML = 'Aplicación creada por: Eliz Longart'
 */
...
function buildToDo(todo, index) {
  // Tu código acá:
/* Este es el todo: 
let nuevaTarea = new toDoItems('hacer homewrok')
nuevaTarea = {
  description: 'hacer homework';
  complete: false;
} */

  let toDoShell = document.createElement('div');
  toDoShell.className = 'toDoShell' //aqui estoy haciendo algo como: toDoShell = <div class='toDoShell> </div>
  
  let toDoText = document.createElement('span');
  toDoText.innerHTML = todo.description //aqui estoy haciendo algo como: toDoText = <span> hacer homework <span>
  toDoText.id = index; //aqui estoy haciendo algo como: toDoText = <span id='index'> hacer homework <span>
...
// 3) En la función 'buildToDo' agregar un 'click' event listener al elemento 'toDoText', pasándole esta función como callback 
 ...
 toDoText.addEventListener('click', completeToDo);
  ...
  if (todo.complete === true){
    toDoText.className = 'completeText'; //aqui estoy haciendo algo como: toDoText = <span id='index' class='completeText'> hacer homework <span>
  }

  toDoShell.appendChild(toDoText); 
 /*  aqui estoy haciendo algo como:
  <div class='toDoShell>
  <span id='index'> hacer homework <span>
   </div> */
...
function buildToDos(toDos) {
  ...
  for (let i = 0; i < array.length; i++) {
    let nuevoTodo = buildToDo(toDos[i], i)
    nuevoArrayDeToDosDiv.push(nuevoTodo)
  }
  return nuevoArrayDeToDosDiv; */

  //con map:

  let nuevoArrayDeToDosDiv = toDos.map(buildToDo);
  return nuevoArrayDeToDosDiv;
}
...
function completeToDo(event) {
  ...
  const indexDelItem = event.target.id;
  // Tu código acá:
